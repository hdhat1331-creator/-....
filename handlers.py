...
# ============ حالت انتظار ادمین ============
WAITING = {}  # user_id -> {"action": "...", "chat_id": ...}
...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- جواب‌های گروه‌های ادمین ----
    if data == "panel_myreplies":
        if not is_admin(uid): return
        await q.answer()
        groups = await get_all_groups()
        if not groups:
            await q.edit_message_text("هیچ گروهی ثبت نشده.", reply_markup=back_button())
            return
        await q.edit_message_text("کدوم گروه؟", reply_markup=group_selector(groups))
        return

    # ---- پیام‌های دوره‌ای گروه‌ها (ادمین) ----
...
async def private_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text.startswith("/"):
        return
    # فقط ادمین اصلی برای پنل ورودی می‌ده
...
async def on_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ============ دستورات مدیریتی (ریپلای) ============
    if msg.reply_to_message and msg.reply_to_message.from_user:
        target = msg.reply_to_message.from_user
        target_id = target.id
        target_name = target.first_name

        # فقط مالک یا ادمین گروه
        ...
        if admin_ok:
            ...
            # سکوت
            if any(cmd in text for cmd in CMD_MUTE) and not any(cmd in text for cmd in CMD_UNMUTE):
                try:
