...
# ---------- تنظیمات مستقیم ----------
BOT_TOKEN = "8778309418:AAEv8Y32v1_8gQkZUHiQqZbQjStmNRAGv6A"
ADMIN_ID = 8235703809
WEBHOOK_URL = "https://hamal-bot-6lpf.onrender.com"
PORT = 10000
...
async def add_reply(update, context):
    ...
    if "|" not in text:
        await update.message.reply_text("فرمت: /add کلید | جواب"); return
...
async def list_replies(update, context):
    ...
    db = load_db()
    if not db["replies"]:
        await update.message.reply_text("خالیه!"); return
    out = "📋 جواب‌ها:\n\n"
...
async def add_interval(update, context):
    ...
    db = load_db(); db["interval_messages"].append(msg); save_db(db)
    await update.message.reply_text(f"⏰ اضافه شد:\n{msg}")

async def clear_interval(update, context):
    if update.effective_user.id != ADMIN_ID: return
    db = load_db(); db["interval_messages"] = []; save_db(db)
...
async def on_group_message(update, context):
    ...
    if msg.reply_to_message and msg.reply_to_message.from_user.id == context.bot.id:
        await smart_reply(update, context); return

    for key, answers in db["replies"].items():
        if key == "__reply_back__": continue
        if key in msg.text:
            await msg.reply_text(random.choice(answers)); return
...
# ---------- دستور setwebhook دستی ----------
async def setwebhook_cmd(update, context):
    if update.effective_user.id != ADMIN_ID: return
    url = f"{WEBHOOK_URL}/{BOT_TOKEN}"
    await context.bot.set_webhook(url)
    info = await context.bot.get_webhook_info()
    await update.message.reply_text(f"✅ وب‌هوک ست شد:\n{info.url}")

async def webhook_info(update, context):
    if update.effective_user.id != ADMIN_ID: return
    info = await context.bot.get_webhook_info()
    await update.message.reply_text(
        f"🔗 URL: {info.url}\n"
        f"📬 Pending: {info.pending_update_count}\n"
        f"⚠️ Last error: {info.last_error_message or 'ندارد'}"
    )
...
async def main_async():
    ...
    app.add_handler(CommandHandler("setwebhook", setwebhook_cmd, filters.ChatType.PRIVATE))
    app.add_handler(CommandHandler("webhook", webhook_info, filters.ChatType.PRIVATE))
