...
async def panel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_home":
        if is_admin(uid):
            ...
            if not groups:
                await q.edit_message_text("⛔ گروهی نداری.")
                return
            if len(groups) == 1:
                g = groups[0]
                await q.edit_message_text(
    ...
    if data == "panel_cleanup":
        if not is_admin(uid):
            await q.answer("دسترسی نداری", show_alert=True)
            return
        await q.answer()
        await q.edit_message_text("🧹 دارم گروه‌های مرده رو چک می‌کنم...")
        removed = await cleanup_dead_groups(context.bot)
        remaining = await get_all_groups()
        await q.edit_message_text(
            f"✅ {removed} گروه مرده پاک شد.\n\n👥 گروه‌های سالم: {len(remaining)}",
...
🎯 و admin_panel.py — دکمه‌ی cleanup رو اضافه کن

فقط تابع main_panel رو عوض کن:
