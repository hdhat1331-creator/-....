...
# ============ کلماتی که ربات بهشون ریپلای می‌زنه ============
TRIGGER_WORDS = ["حمال", "حمالل", "hamal"]
...
async def games_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "game_rps":
        await q.edit_message_text("انتخاب کن:", reply_markup=rps_menu())
        return
...
async def on_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in GUESS_GAMES and msg.text.strip().isdigit():
        ...
        if guess == target:
            del GUESS_GAMES[uid]
            await msg.reply_text(f"درست حدس زدی: {target}")
        elif guess < target:
            await msg.reply_text("بالاتره")
        else:
            await msg.reply_text("پایین‌تره")
        return

    # ---- ریپلای روی پیام ربات → تیکه سنگین ----
    if msg.reply_to_message and msg.reply_to_message.from_user.id == context.bot.id:
        # اگه ادمین جواب سفارشی "__reply_back__" گذاشته باشه، از اون استفاده کن
        custom = db["replies"].get("__reply_back__")
        if custom:
            await msg.reply_text(random.choice(custom))
        else:
            await msg.reply_text(random.choice(ROASTS))
        return

    # ---- کلمات کلیدی سفارشی (از پنل ادمین) ----
